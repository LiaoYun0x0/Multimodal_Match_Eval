...
class GLDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        sar_img_path  = self.train_data[index].rstrip('\n')
        sar_img = cv2.imread(sar_img_path)
        sar_img = cv2.cvtColor(sar_img,cv2.COLOR_BGR2RGB)

        opt_img_path = os.path.join(os.path.dirname(sar_img_path) + '/' + os.path.basename(sar_img_path).replace('sar_', ''))
        ...
        #temp_img = np.zeros_like(sar_img)
        #temp_img[:400, :400, ...] = sar_img[50:450, 50:450, :]
        #sar_img = temp_img.copy()
        #rotation_matrix = cv2.getRotationMatrix2D((256, 256), 15, 1)
        #h, w, c = opt_img.shape
        #rotated_image = cv2.warpAffine(sar_img, rotation_matrix, (w, h))
        #sar_img = rotated_image.copy()
